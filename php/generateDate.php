...
function createPresentationSlots($duration, $start, $end, $date, $room)
{
    ...
    if ($count == 0) {
        ...
        while (strtotime('+' . $duration . ' minutes', strtotime($start_time)) <= strtotime($end_time)) {
            ...
            insertDateSlots($start_time, $end, $duration, $room);
            $start_time = $end;
            $hours++;
        }
        if ($hours == 0) {
...
$result = createPresentationSlots($durationPresentation, $startDate, $endDate, $date, $room);
echo $result;
function insertDateSlots($date, $end, $durationPresentation, $room)
{
    try {
        $conn = dbConnection();
        $sql = "INSERT INTO dates (timeDate, timeEnd, duration, room) values (:datePlaceholder, :timeEndPlaceholder, :durationPlaceholder, :roomPlaceholder);";
        $stmt = $conn->prepare($sql);
